...
public class MegamapTracker
{
    ...
    public void fillUpBuffer(String saveFolder, long time, PlayerEntity player)
    {
        //save player data to the arrays
        xPositions[tag] = player.x;
        zPositions[tag] = player.z;
        dimensions[tag] = player.world.dimension.dimensionType; //public in 1.7, no getter needed
...
//                String saveDirectory = "saves/" + saveFolder + "/";
            ...
            {
                System.out.println(String.format("Could NOT write data in %s. Overwriting buffer", saveFolder));
                tag = 0;
            }
